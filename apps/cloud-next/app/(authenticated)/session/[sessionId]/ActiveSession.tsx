...
import { createSpringEffect } from '~/lib/create-spring-effect'

const FULL_LIP_HEIGHT = 108
const FULL_ACTION_LIP_BOX_HEIGHT = 124
const MD_BREAKPOINT = 768
const MAX_CONTAINER_WIDTH = 1280
const HORIZONTAL_DRAG_ACTION_THRESHOLD = 96
const DRAG_DROP_LIST_HEADER_HEIGHT_MOBILE = 144
const DRAG_DROP_LIST_HEADER_HEIGHT_DESKTOP = 160
const NO_SHOW_THRESHOLD = 1000 * 60 * 3
const SESSION_END_CHECK_INTERVAL = 1000 * 10
...
export function ActiveSession({ initialSession }: { initialSession: Session }) {
        ...
        void utils.lip.getBySessionId.setData(
            ...
            (prevLips) => {
                ...
                const update = prevLips
                    .map((lip) => {
                        if (lip.sortNumber === null) return lip
                        if (lip.id === id) {
                            return { ...lip, status, sortNumber }
                        }
                        ...
                        if (fromLip.status === status) {
                            if (lip.status !== status) return lip
